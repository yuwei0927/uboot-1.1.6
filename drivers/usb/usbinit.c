...
{    
	...
	gpioregs->GPCDAT &= ~(1<<5);	/* disable USB Device, enable later, mike.arm9 */
    ...
    //rGPGCON = 0xff95ffba;
    gpioregs->GPGCON = 0xfd95ffba;	/* mike.arm9, GPG12-->OUTPUT, as USB Device enable */
    gpioregs->GPGUP  = 0xffff;    // The pull up function is disabled GPG[15:0]
	gpioregs->GPGDAT &= ~(1<<12);	/* disable USB Device, enable later, mike.arm9 */
