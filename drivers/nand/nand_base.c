...
static int nand_block_bad(struct mtd_info *mtd, loff_t ofs, int getchip)
{
	...
	if (this->options & NAND_BUSWIDTH_16) {
        ...
        // add by www.arm9.net, if not, the erase will be failed
		if (!this->dev_ready)
		...
		int autoplace, int numpages)
{
	...
	/* If we have no autoplacement or no fs buffer use the internal one */
        if (!autoplace || !fsbuf) {
                this->oobdirty = 1;                     // by mike.arm9, www.arm9.net
                return this->oob_buf;
        }
...
/* XXX U-BOOT XXX */
	...
	return 0; //this->scan_bbt (mtd); // cancelled by www.arm9.net, scan_bbt uses more time
}
